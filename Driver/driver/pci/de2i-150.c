...
static long int my_ioctl(struct file* dummy_file, unsigned int cmd, unsigned long arg)
{
	switch(cmd){
	case RD_SWITCHES:
		read_pointer = bar0_mmio + 0xF000; //DONE
		rd_name_idx = IDX_SWITCH;
		break;
	case RD_PBUTTONS:
		read_pointer = bar0_mmio + 0xD000; //DONE
		rd_name_idx = IDX_PBUTTONS;
		break;
	case WR_L_DISPLAY:
		write_pointer = bar0_mmio + 0xC040; //DONE
		wr_name_idx = IDX_DISPLAYL;
		break;
	case WR_R_DISPLAY:
		write_pointer = bar0_mmio + 0xC000; //DONE
		wr_name_idx = IDX_DISPLAYR;
		break;
	case WR_RED_LEDS:
		write_pointer = bar0_mmio + 0xC060; //DONE
		wr_name_idx = IDX_DISPLAYR;
		break;
	case WR_GREEN_LEDS:
		write_pointer = bar0_mmio + 0xC080; //DONE
		wr_name_idx = IDX_DISPLAYR;
		break;
	default:
